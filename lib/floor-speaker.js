...
(function (root) {
  ...
  function matchSurname(surname, stateFullName, roster, gender) {
    ...
    for (let p = 0; p < pools.length; p++) {
      ...
      for (const r of pool) {
        ...
        if (d > cap) continue;
        // An honorific that disagrees rules out an INEXACT match. "I NOW YIELD TO
        // THE DISTINGUISHED GENTLEMAN FROM NEW YORK, MR. FENIG" was resolving to
        // Grace Meng — two edits, inside the allowance for a five-letter surname,
        // and she is a gentlewoman. An exact spelling still wins regardless, since
        // the captions get an honorific wrong often enough on its own.
  ...
  function buildPatterns(roster) {
    ...
    return {
      ...
      embeddedRecognition: new RegExp(
        ...
        GAP + '{0,60}?(?:IS\\s+)?(?:NOW\\s+)?RECOGNIZED[^.]{0,40}\\.', 'i'),
      ...
      clerkCall: /\bCLERK\s+WILL\s+(?:REPORT|DESIGNATE|READ)\b/i,
  ...
  const CHAIR_LEADIN = /^(?:PURSUANT\s+TO|UNDER\s+THE\s+SPEAKER|WITHOUT\s+OBJECTION|FOR\s+WHAT\s+PURPOSE|FOR\s+PURPOSES?\b|THE\s+CHAIR\b|BY\s+DIRECTION|THE\s+HOUSE\s+WILL|THE\s+QUESTION\s+IS|GENTLE\w+\s+(?:YIELDS|RESERVES))/i;
  ...
  function resolveFloorSpeakers(turns, roster, opts) {
    ...
    const bindFrom = (text) => {
      ...
      const offer = (rawState, surname, hon) => {
        ...
        take(st, matchSurname(surname, st, roster, genderOfHonorific(hon)));
      };
      const offerTokens = (rawState, tokens, hon) => {
        const st = canonicalState(rawState, roster);
        take(st, matchPersonTokens(tokens, st, roster, genderOfHonorific(hon)));
      };
      P.bind.lastIndex = 0;
      let m;
    ...
    for (const turn of splitEmbeddedChairTurns(turns, P)) {
      ...
      if (P.adjourned.test(text) || P.recessed.test(text)) {
        ...
        floor = null; floorBasis = null; pending = null; pendingUnknown = false;
        hourHolder = null; bind = {}; managerStates = []; carried = 0;
      ...
      if (named && (yieldsAway || invites)) { pending = { member: named.member, state: named.state }; pendingUnknown = false; }
      ...
      let confidence = null;
      if (floor) {
        confidence = { 'chair-named': 0.95, 'yielded-named': 0.95, 'manager-binding': 0.9, 'sole-delegate': 0.85, 'hour-holder': 0.7 }[floorBasis] || 0.7;
    ...
    const lastSpeech = [...timeline].reverse().find((x) => x.role === 'speech' || x.role === 'clerk');
  ...
  function resolveLiveFloor(text, roster, seedBind) {
    ...
    for (const e of events) {
      ...
      if (e.kind === 'clerk') {
        floor = null; basis = 'clerk'; at = e.index; lastNamed = null;
        continue;
      }
      if (e.kind === 'adjourned' || e.kind === 'recess') {
        floor = null; basis = e.kind; at = e.index; lastNamed = null;
        continue;
      }
    ...
    if (basis === 'clerk') return { member: null, basis, fromEnd: text.length - at };
  ...
  root.HouseFloorSpeaker = {
    resolveLiveFloor, dedupeLiveCues, matchPersonTokens, cleanUntil, splitEmbeddedChairTurns,
