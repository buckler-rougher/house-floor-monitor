...
(function (root) {
  ...
  function matchPersonTokens(tokens, stateFullName, roster) {
    const parts = String(tokens || '').trim().split(/\s+/).filter(Boolean);
    if (!parts.length) return null;
    const surname = parts[parts.length - 1];
    const direct = matchSurname(surname, stateFullName, roster);
  ...
  function resolveFloorSpeakers(turns, roster, opts) {
    ...
    const bindFrom = (text) => {
      ...
      const offer = (rawState, surname) => {
        ...
        take(st, matchSurname(surname, st, roster));
      };
      const offerTokens = (rawState, tokens) => {
        const st = canonicalState(rawState, roster);
        take(st, matchPersonTokens(tokens, st, roster));
      };
      P.bind.lastIndex = 0;
      let m;
      while ((m = P.bind.exec(text)) !== null) offer(m[1], m[2]);
      P.nameOfState.lastIndex = 0;
      while ((m = P.nameOfState.exec(text)) !== null) offerTokens(m[2], m[1]);
      let found = null;
    ...
    for (const turn of turns) {
      ...
      if (named && /YIELD|INVITE/i.test(text)) { pending = { member: named.member, state: named.state }; pendingUnknown = false; }
  ...
  function resolveLiveFloor(text, roster, seedBind) {
    ...
    P.nameOfState.lastIndex = 0;
    while ((m = P.nameOfState.exec(text)) !== null) push(m.index, 'tokens', m[2], m[1]);
    ...
    for (const e of events) {
      if (e.kind === 'name' || e.kind === 'tokens') {
        ...
        const hit = e.kind === 'tokens'
          ? matchPersonTokens(e.surname, st, roster)
          : matchSurname(e.surname, st, roster);
