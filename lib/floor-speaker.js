...
(function (root) {
  ...
  function buildPatterns(roster) {
    ...
    return {
      ...
      yieldNamed: new RegExp(PERSON + '\\s+FROM\\s+(' + ST + ')[\\s\\S]{0,200}?(' + HON + ')\\s+([A-Z][A-Z\'\u2019-]{2,})', 'gi'),
  ...
  function resolveFloorSpeakers(turns, roster, opts) {
    ...
    for (const turn of splitEmbeddedChairTurns(turns, P)) {
      ...
      if (pending) {
        ...
        floor = null; floorBasis = 'yield-unresolved'; pendingUnknown = false; carried = 0;
      }
      ...
      if ((yieldsAway || invites) && failedStates.size) {
        for (const st of failedStates) {
          if (!named || named.state !== st) { delete bind[st]; unnamedYieldTo.add(st); pendingUnknown = true; }
        }
      }
  ...
  // to still be the latest one. When it is, the live track decides. When it is
  // not, the server does — it has had time to catch up by then.
  function floorRole(live, liveFresh, current) {
    if (liveFresh && live) {
      if (live.basis === 'clerk') return 'clerk';
      return 'member';   // a live event this recent outranks the server's role
    }
    if (current && current.role === 'chair') return 'chair';
    if (current && current.role === 'clerk') return 'clerk';
    return 'member';
  }

  root.HouseFloorSpeaker = {
    clerkActionPhrase, floorRole,
