...
// ── Which chamber is this board ──────────────────────────────────────────────
//
// One codebase, two hostnames. Every recurring title on the board that names a
// chamber lives here, so a Senate build overrides this object rather than
// hunting through 12,000 lines for the word "House".
//
// Only the repeating titles are here. One-off prose -- the sine die line, the
// question-of-privileges line, the info-popup bodies -- is not chamber-variable
// so much as chamber-specific writing, and a Senate build wants its own copy of
// those rather than a swapped noun.
//
// The values below are exactly what the board has always rendered, so the House
// output is unchanged by this indirection.
const CHAMBERS = {
    house: {
        id: 'house', name: 'House',
        chaplainTitle:  'House Chaplain',
        presidingTitle: 'Speaker of the House',
        readingClerk:   'House Reading Clerk',
        teller:         'House teller',
        officers:       'Officers of the House',
        dean:           'Dean of the House',
    },
    senate: {
        id: 'senate', name: 'Senate',
        chaplainTitle:  'Senate Chaplain',
        presidingTitle: 'President pro tempore',
        readingClerk:   'Senate Reading Clerk',
        teller:         'Senate teller',
        officers:       'Officers of the Senate',
        // The Senate has no Dean; the President pro tempore administers the oath.
        dean:           'President pro tempore',
    },
};

// The hostname decides, because one Pages project serves both domains. The
// markup carries data-chamber="house" so CSS has it before this runs; anything
// unrecognised stays the House, which is what the board is today.
const CHAMBER = CHAMBERS[/(^|\.)senate-/.test(location.hostname) ? 'senate' : 'house'];
try { document.body?.setAttribute('data-chamber', CHAMBER.id); } catch (_) {}

const MAJORITY_PARTY_FALLBACK = 'R';
function majorityParty() {
    return (controllingParty === 'R' || controllingParty === 'D')
        ? controllingParty
        : MAJORITY_PARTY_FALLBACK;
}
...
function getPartyPrefs(party) {
    const isMajority = party === majorityParty();
    if (party === 'D') {
...
const INFO_CONTENT = {
    'prayer': {
        ...
        body: () => {
            const name = _lastChaplainName || `the ${CHAMBER.chaplainTitle}`;
...
function updatePrayerSection(items) {
    ...
    if (!prayerItem) {
        ...
        if (guest || house) {
            ...
            elements.prayerLeaderTitle.textContent = guest ? 'Guest Chaplain' : CHAMBER.chaplainTitle;
    ...
    elements.prayerLeaderTitle.textContent = isGuestChaplain ? 'Guest Chaplain' : CHAMBER.chaplainTitle;
...
async function updateJournalSection(items) {
    ...
    elements.journalChairTitle.textContent = CHAMBER.presidingTitle;
...
async function fetchJournalSpeakerInfo() {
    try {
        ...
        if (elements.journalChairName) elements.journalChairName.textContent = CHAMBER.presidingTitle;
    }
}
...
async function fetchSpeakerAsChair() {
    try {
        ...
        // Photo from BioGuide
        const photoUrl = buildBioguidePhotoUrl(bioguideId);
        setMemberPhoto(elements.pledgeImage, photoUrl, name || CHAMBER.presidingTitle);
        const profileUrl = buildCongressProfileUrl(bioguideId);
        setMemberProfileLink(elements.pledgeLeaderWebsite, profileUrl);
    } catch (error) {
        console.error('fetchSpeakerAsChair failed:', error);
        if (elements.pledgeLeaderName) elements.pledgeLeaderName.textContent = CHAMBER.presidingTitle;
    }
}
...
async function fetchTellerInfo(nameStr, cardEl) {
    try {
        ...
        if (cardEl) {
            ...
            cardEl.querySelector('.teller-meta-line').textContent = town || CHAMBER.teller;
...
async function fetchMemberPhotoFromClerkData(leaderName) {
    try {
        ...
        if (bestMatch && bestMatch.bioguideId) {
            ...
            setMemberPhoto(elements.pledgeImage, photoUrl, bestMatch.fullName || CHAMBER.presidingTitle);
            return;
        }
...
(function initFloorSpeaker() {
    ...
    function render(data) {
        ...
        if (role === 'clerk') {
            ...
            name.textContent = CHAMBER.readingClerk;
